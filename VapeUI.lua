--情云的VAPE UI
local UI = {RainbowColorValue = 0, HueSelectionPosition = 0}
...
coroutine.wrap(
    function()
        while wait() do
            UI.RainbowColorValue = UI.RainbowColorValue + 1 / 255
...
function UI:Window(text, preset, closebind)
    ...
    while true do
        for v = 0, num do
            local c = Color3.fromRGB(127 * math.sin(w*v + counter) + 128, 127 * math.sin(w*v + 2 * math.pi/3 + counter) + 128, 127*math.sin(w*v + 4*math.pi/3 + counter) + 128)
            table.insert(CS, v+1, ColorSequenceKeypoint.new(v/num, c))
        end
    ...
    if (counter >= math.pi * 2) then counter = 0
        end
    end
    ...
    Main:TweenSize(UDim2.new(0, 560, 0, 320), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.6, true)
    ...
    function UI:ChangePresetColor(toch)
        PresetColor = toch
    end

    function UI:Notification(texttitle, textdesc, textbtn)
        ...
        NotificationHold.Size = UDim2.new(0, 560, 0, 320)
                ...
                function()
                    ...
                    if RainbowColorPicker then
                        ...
                        while RainbowColorPicker do
                            BoxColor.BackgroundColor3 = Color3.fromHSV(UI.RainbowColorValue, 1, 1)
                            Color.BackgroundColor3 = Color3.fromHSV(UI.RainbowColorValue, 1, 1)

                            ColorSelection.Position = UDim2.new(1, 0, 0, 0)
                            HueSelection.Position = UDim2.new(0.48, 0, 0, UI.HueSelectionPosition)
